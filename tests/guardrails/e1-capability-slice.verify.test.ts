...
describe("E1 capability slice verification", () => {
  it.effect("executes the public foundation-core capability slice and emits durable evidence", () =>
    Effect.gen(function* () {
      ...
      expect(inspection.nextStepId).toBe("step-extract-001");
      expect(inspection.progress.completedSteps).toBe(1);
      expect(inspection.progress.pendingSteps).toBe(2);
      expect(inspection.progress.completedStepIds).toEqual(["step-capture-001"]);
      expect(inspection.progress.pendingStepIds).toEqual(["step-extract-001", "step-snapshot-001"]);
      expect(inspection.progress.completionRatio).toBe(1 / 3);
      expect(inspection.budget.checkpointInterval).toBe(2);
      expect(inspection.budget.stepsUntilNextCheckpoint).toBe(2);
      expect(inspection.budget.remainingTimeoutMs).toBe(0);
      expect(inspection.budget.timeoutUtilization).toBe(1);
