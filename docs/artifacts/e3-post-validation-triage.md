...
| --- | --- | --- | --- | --- |
| Response body-read failures happen after a successful fetch but still bypass the retry loop, so a transient stream-read failure can lose the capture even when retry budget remains. | Medium reliability gap | `docs/runbooks/e3-retry-backoff-runbook.md` documents this current limitation, and `tests/libs/foundation-core-e3-runtime.test.ts` exercises the terminal `body read failed` path. | Do first. Fix before downstream phases lean harder on HTTP-first capture or assume retry coverage spans the full capture path. | `bd-afb.37` |
